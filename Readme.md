...
- **AI brain:** Anthropic Claude API (use the `anthropic` Python SDK, model: a current
  Claude model suitable for chat; keep the model name in config so it is easy to change)
...
│   ├── ai.py              # Claude API calls, prompt assembly
...
- `AI_MODEL` — Claude model name string
